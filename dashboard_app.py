...
with st.expander(f"Perbandingan algoritma Load Forecast (model terbaik: {LOAD_RES['best_model_name']})"):
    ...
    st.dataframe(pd.DataFrame(algo_rows).sort_values("MAPE (%)"), use_container_width=True, hide_index=True)
...
def run_grid_cached(_n_scenarios):
    rng = np.random.default_rng(C.RANDOM_STATE)
    rows = []
    for r_mrc in L2.RASIO_MRC_GRID:
        sim_g = L2.run_scenarios_for_ratio(SIM_POOL, COAL_GCV, SFC_MODEL, RESULTS, r_mrc, _n_scenarios, rng)
        rows.append(L2.summarize_ratio(sim_g, r_mrc))
    return pd.DataFrame(rows)


grid_df = run_grid_cached(n_scenarios)
...
with gc2:
    st.write("")
    st.button("\U0001F3AF Terapkan Skenario Optimal", use_container_width=True,
...
if val_df is None or val_df.empty:
    ...
    st.dataframe(val_df[show_cols], use_container_width=True, hide_index=True)
